...
def sync_xray_core():
    ...
    if has_mtproto_proxy:
        subprocess.run(f"sudo fuser -k {TELEGRAM_PROXY_PORT}/tcp || true", shell=True)
    ...
    if os.path.exists(xray_bin) or shutil.which(xray_bin):
        subprocess.run(f"sudo nohup {xray_bin} -config {XRAY_CONFIG_PATH} > /dev/null 2>&1 &", shell=True)
...
<script>
    ...
    function switchPanelTab(tabId) {{
        const tabs = ['dashboard','clients','combo_subs','tg_configs','telegram_settings','system_settings','advanced_configs','terminal','logs','dpi'];
        tabs.forEach(t => {{
